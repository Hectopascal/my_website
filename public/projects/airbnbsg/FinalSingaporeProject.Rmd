---
title: "Predicting AirBnB Prices in Singapore"
author: "Group 5: Etra Bianco, Himanshu Singh, Yi Yan Ng, Mengze Sun, Vasu Dev Puri, Christian Gonder"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
# Executive Summary
In this report we will be analysing data about Airbnb listings in Singapore with the goal of building a model to predict the total cost for two people staying four nights in Singapore. The data is from [insideairbnb.com]()

**Purpose & Scope:** the purpose of this report is to elucidate upon the set deliverable across five facets:

1. Perform a cohesive exploratory data analysis;
2. Chart data in plots and geospatial format to discern trends and select explanatory variables for regression purposes;
3. Conduct a robust regression analysis with selected variables;
4. Diagnose variables demonstrating collinearity, replace said variables to curtail "Variance Inflation Factor", summarize shortlisted data sets for predictive and informative purposes.
5. Employ insights obtained from conducting the analysis above to predict the total cost for two people spending four nights in Singapore. 

**Methodology & Design**: the underlying data set provided information across a wide spectrum, pertaining to both the quantitative and qualitative aspects of AirBnB listings in Singapore. A high level analysis using the `glimpse` & `skim` functions resulted in the ostracization of certain variables pertaining to either the qualitative or unvaried data points with an aim to reduce the duplicity and multicollinearity.

Shortlisted data, albeit concise, still experienced missing, incorrect & null values. In order to make the data fit for purpose, a robust parsing exercise was conducted to enhance the data’s usefulness and comprehension.
Subsequently, visualizations in the form of mappings, analytics in the form of regressions, and summarisation in the form of data tables were conducted for analytical and commentative purposes.

**Key Considerations**: regression analysis was not conducted upon all variables in the data set due to duplicative data points, lackluster explanatory power, essence of brevity, and time-sensitive nature of the engagement.
Missing values and N/As were replaced with numeric vectors, on a case by case basis, after conducting desktop research on AirBnB's operational & business methodology.
Correlations / high regression coefficients may arise outside the data bucket created, since variables clustered based on a high-level review of their descriptions. 
Listings requiring a minimum of five nights were excluded, as sufficient data points were available to compute a four night stay.

**Summary of findings** 
From our analysis there was no apparent stong correlation between our quantitative explanatory variables and the price for four nights. However, albeit antithetical, our model hints that categorical variables possess a higher explanatory power as opposed to the numerical ones. We understand that the categorical nature of the data points - Property type, neighborhood, etc. - have a noticeable impact on the travel and tourism industry. 

**Predictions** 
From our regression analysis, we predict with a 95% level of conficenve that the cost for 2 people of staying in an AirBnB in Singapore for four nights would be approximately US$338. However, we acknowledge that this is subject to the limitations of our analysis, including gaps in our data, subjectivity in selecting variable and the potentially skewing effect of the Covid-19 pandemic. 

```{r load data}
#importing the raw data from insideairbnb.com 
#used read_csv as vroom caused a column mismatch
df_raw <- read_csv(here("content/projects/airbnbsg/data","listings.csv"))
```
...
vif(model1)
```
We look at the first graph, "Residuals VS Predicted ", to confirm or disprove the linear relationship that we are assuming exists between our x and y variable. The pattern in the plot above confirms the linear relationship as the residuals seem equally spread around a horizontal line.
With regards to our second chart, "Normal Q-Q" plot, this checks if our residuals follow a normal distribution, which they do as they roughly follow the diagonal straight line.
...
Lastly, the "Residuals VS Leverage", highlight any influential data points in our model. Outliers are defined as the points with a high y value and such points on our model all have low x values, thus having little leverage on our model. 
...
We see that the adjusted R-squared score has increased greatly (to 45%) and the p-values are small across all `room_type` variables. Those statistics indicate that `room_type` is indeed a good predictor of price, and we should keep it into our model. Meanwhile, VIFs are also encouraging, indicating a lack of collinearity across variables. 

### Further exploration

Even though the adj R square improved a lot by adding room_type, it is still only 0.45(<0.5). We want to further investigate the significance of the number of bathrooms, beds, bedrooms and the size of the house. 
```{r, fig.height = 10, fig.width=10}
...
vif(model1)
```
After adding these variables, adjusted R-squared increases to 0.56. This model now is able to explain 56% of the variance of `price_4_night`. However, we note that the P value of the East Region neighbourhood is not significant. As a consequence of this finding we adapt the model further by adapting the neighbourhood value. 
...
```
From table above, all the red number means we should not include these variables into our model. Since there are an excessive number of possible values for `neighbourhood_cleansed`, we explore by replacing `neighbourhood_cleansed` with `neighbourhood_group_cleansed` (which divides neighbourhood into regions) to reduce the number of factors. This produces a much cleaner model, but causes a lower adjusted R-squared score shown below.
```{r}
...
```
Now the adjusted R-squared decreases from 0.63 to 0.56. We thus try to keep the more significant factors and drop the unnecessary ones. To do this, we first convert the `neighbourhood_cleansed` categorical variable into dummy variables (one-hot encoding). 
...
From our summary table, we can say most of our P values are significant while maintaining R square in a relatively high number.

## Prediction

We are going to use our final model to predict with a 95% confidence interval how much it will cost 2 people to stay 4 nights in Singapore, specifically, sleeping at an apartment with a private room with at least 10 reviews, and an average rating of at least 90.
...
```
Above is our final prediction for 2 people staying 4 nights in Singapore. Our calculations show a mean for the predicted price of $338 with a lower and upper 95% confidence interval of \$91.3 and \$584. This is a fairly reasonably price for 4 nights in Singapore for AirBnB accommodation.

# Limitation

Regression analysis, a key component of our workings, explains a linear relationship between dependent and independent variables. While we have selected the variables that produce the best linear model, our understanding is that travel and tourism is impacted by qualitative variables as well, which may not be explainable via a regression model.

The data contained numerous missing values, which were mostly excluded during the cleaning of the data, causing our initial 7323 observations to be reduced to 2444 observations. While we attempted to preserve the relevant information, omission of these observations may still cause error in the model built upon the remaining 30% of observed values. Moreover, most of our variables were categorical variables and there were only a limited number of numerical values to use for the linear regression. 

As a limitation to our analysis above it is important to note that the information provided is as of June 2020, hence the data and our subsequent findings are profoundly skewed by the Covid-19's impact on travel & tourism. Another aspect that should be taken into account is seasonality, namely this model reflects demand for AirBnBs in the month of June, however the demand and price points might fluctuate according to the time of the year depending on peak travel times such as over longer holidays that bring and inflow of tourism. 

Lastly, the reliability and accuracy of our source for the raw data is questionable, given that the data was extracted from the website [insideairbnb.com](), which is an unaccounted for and unknown author.  

# Conclusion 

In conclusion, from our exploratory Data Analysis and Regression Analysis of a sample of AirBnB listings in Singapore, we can predict with a 95% confidence that a 4 night stay for 2 people in the city would have a total cost of about US$338.  

# Appendix 

## Appendix 1 

Glimpse and Skim of the raw data frame
```{r, cache=TRUE }
#glimpsing the data frame to get a sense of the raw data
glimpse(df_raw)
```
...
Glimpse and Skim of the final data frame after data cleaning  
...
For reference you can find find below the complete collinearity.
...
Output of initial model constructed using neighborhood p-values. The insignificant neighborhood p-values are highlighted in red in the output.
